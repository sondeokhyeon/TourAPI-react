...
export async function getDetailInfo(arg) {
  const { item, id } = arg;
  const url = `contentTypeId=${item}&contentId=${id}&MobileOS=ETC&MobileApp=AppTest&defaultYN=Y&firstImageYN=Y&areacodeYN=Y&catcodeYN=Y&addrinfoYN=Y&mapinfoYN=Y&overviewYN=Y&transGuideYN=Y`;
